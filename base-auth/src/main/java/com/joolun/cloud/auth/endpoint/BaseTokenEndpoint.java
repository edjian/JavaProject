package com.joolun.cloud.auth.endpoint;
...
public class BaseTokenEndpoint {
	...
	public R<Page> tokenList(@RequestBody Map<String, Object> params) {
		...
		baseRedisTemplate.setKeySerializer(new StringRedisSerializer());
		baseRedisTemplate.setValueSerializer(new JdkSerializationRedisSerializer());
		Page result = new Page(MapUtil.getInt(params, PaginationConstants.CURRENT), MapUtil.getInt(params, PaginationConstants.SIZE));
		result.setRecords(baseRedisTemplate.opsForValue().multiGet(pages));
		result.setTotal(Long.valueOf(baseRedisTemplate.keys(key).size()));
		return R.ok(result);
	}
	...
	private List<String> findKeysForPage(String patternKey, int pageNum, int pageSize) {
		...
		RedisSerializer<String> redisSerializer = (RedisSerializer<String>) baseRedisTemplate.getKeySerializer();
		Cursor cursor = (Cursor) baseRedisTemplate.executeWithStickyConnection(redisConnection -> new ConvertingCursor<>(redisConnection.scan(options), redisSerializer::deserialize));

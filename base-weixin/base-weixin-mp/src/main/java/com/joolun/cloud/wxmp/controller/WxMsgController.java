...
import com.joolun.cloud.wxmp.entity.WxMsg;
import com.joolun.cloud.wxmp.entity.WxMsgVO;
...
import com.joolun.cloud.wxmp.service.WxMsgService;
import com.joolun.cloud.wxmp.service.WxUserService;
...
public class WxMsgController {
    ...
    public R getWxMsgPage(Page page, WxMsgVO wxMsgVO) {
    	...
    	if(StringUtils.isNotBlank(wxMsgVO.getWxUserId())){//标记为已读
			...
			Wrapper queryWrapper = new QueryWrapper<>()
					.eq("wx_user_id",wxMsgVO.getWxUserId())
					.eq("read_flag",CommonConstants.NO)
					.eq("app_id",wxMsgVO.getAppId());
    ...
    */
    @GetMapping("/{id}")
    @PreAuthorize("@pms.hasPermission('wxmp_wxmsg_get')")
    ...
    */
    @SysLog("新增微信消息")
    @PostMapping
    @PreAuthorize("@pms.hasPermission('wxmp_wxmsg_add')")
    public R save(@RequestBody WxMsg wxMsg){
		try {
			...
			wxMsg.setCreateDate(LocalDateTime.now());
    ...
    */
    @SysLog("修改微信消息")
    @PutMapping
    @PreAuthorize("@pms.hasPermission('wxmp_wxmsg_edit')")
    ...
    */
    @SysLog("删除微信消息")
    @DeleteMapping("/{id}")
    @PreAuthorize("@pms.hasPermission('wxmp_wxmsg_del')")

...
import com.joolun.cloud.wxmp.entity.WxAutoReply;
import com.joolun.cloud.wxmp.service.WxAutoReplyService;
...
public class WxAutoReplyController {
    ...
    */
    @GetMapping("/page")
    @PreAuthorize("@pms.hasPermission('wxmp_wxautoreply_index')")
    ...
    */
    @GetMapping("/{id}")
    @PreAuthorize("@pms.hasPermission('wxmp_wxautoreply_get')")
    ...
    */
    @SysLog("新增消息自动回复")
    @PostMapping
    @PreAuthorize("@pms.hasPermission('wxmp_wxautoreply_add')")
    ...
    */
    @SysLog("修改消息自动回复")
    @PutMapping
    @PreAuthorize("@pms.hasPermission('wxmp_wxautoreply_edit')")
    ...
    */
    @SysLog("删除消息自动回复")
    @DeleteMapping("/{id}")
    @PreAuthorize("@pms.hasPermission('wxmp_wxautoreply_del')")
	...
	public void jude(WxAutoReply wxAutoReply){
		if(ConfigConstant.WX_AUTO_REPLY_TYPE_2.equals(wxAutoReply.getType())){
			Wrapper<WxAutoReply> queryWrapper = new QueryWrapper<WxAutoReply>()
					.eq("req_type",wxAutoReply.getReqType())
					.eq("app_id",wxAutoReply.getAppId());
		...
		if(ConfigConstant.WX_AUTO_REPLY_TYPE_3.equals(wxAutoReply.getType())){
			Wrapper<WxAutoReply> queryWrapper = new QueryWrapper<WxAutoReply>()
					.eq("req_key",wxAutoReply.getReqKey())
					.eq("rep_mate",wxAutoReply.getRepMate())
					.eq("app_id",wxAutoReply.getAppId());
			List<WxAutoReply> list = wxAutoReplyService.list(queryWrapper);
			if(list != null && list.size() == 1){
				if(!list.get(0).getId().equals(wxAutoReply.getId())){
